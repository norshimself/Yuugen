...
export class EconomyService implements OnModuleInit {
      ...
      const defaultItems = [
        {
          ...
          name: 'VIP Role',
          price: 1000,
          description: 'Get the shiny VIP role in the server!',
        },
        {
          id: 'custom_color',
          name: 'Custom Color',
        ...
        },
        {
          ...
          name: 'Profile Badge',
          price: 250,
          description: 'A cool badge on your profile!',
        },
      ];
  ...
  async startTriviaSession(
    ...
    const session = this.triviaSessionRepository.create({
      userId,
      correctIndex,
      reward,
    });
  ...
  async crime(userId: string): Promise<{
    success: boolean;
    won: boolean;
    amount: number;
    nextClaim?: Date;
  ...
  async buyItem(
    ...
    if (profile.coins < item.price) {
      return {
        success: false,
        message: `You don't have enough coins! You need **${item.price}** coins.`,
      };
    }
